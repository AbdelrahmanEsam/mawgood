...
public class AsyncFingerprint extends Handler {
    ...
    private static final int PS_SET_IR_ENABLE = 0x53;
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case PS_GETIDCARDNO:
                    ...
                    break;
                case PS_SET_IR_ENABLE:
                    int enable = PSSetIREnable();
                    AsyncFingerprint.this.obtainMessage(PS_SET_IR_ENABLE, enable, -1)
                            .sendToTarget();
                    break;
                default:
                    break;
    ...
    public void FP_SetIREnable() {
        mWorkerThreadHandler.sendEmptyMessage(PS_SET_IR_ENABLE);
    }

    private synchronized int PSSetIREnable () {
        byte[] command = {(byte) 0xef, (byte) 0x01, (byte) 0xff, (byte) 0xff, (byte) 0xff, (byte)
                0xff, 0x01, 0x00, 0x03, 0x53, 0x00, 0x05};
        sendCommand(command);
        int length = SerialPortManager.getInstance().read(buffer, 50);
        printlog("PSSetIREnable", length);
        if (length == 12) {
            return buffer[9];
        }
        return -1;
    ...
    private synchronized byte[] PSUpImage() {
        ...
        sendCommand(command);
        int length = SerialPortManager.getInstance().read(buffer, 100);
        Log.d("AsyncFingerprint", "length:" + length);
