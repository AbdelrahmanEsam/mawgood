package com.iraqsoft.mawgood.presentation.fragments.enterYourCodeFragment
...
class EnterYourCodeFragment : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        ...
        binding.viewModel = (requireActivity() as MainActivity).mainViewModel
        return binding.root
    }
    ...
    {
        (requireActivity() as MainActivity).mainViewModel.empNeedsToBeSynced.observe(viewLifecycleOwner,{
    ...
    override fun onDestroyView() {
        super.onDestroyView()
        (requireActivity() as MainActivity).mainViewModel.enteredCode.value = ""
    }

    private fun cacheButtonListener()
    {
        (requireActivity() as MainActivity).mainViewModel.getCachedBranches()
        binding.cacheButton.setOnClickListener {
            if(  (requireActivity() as MainActivity).mainViewModel.enteredCode.value.isNullOrEmpty()){
                toast(requireContext(),"")
            }else{
                if (  (requireActivity() as MainActivity).mainViewModel.cachedBranches.value?.get(0)?.code.isNullOrEmpty()){
                     (requireActivity() as MainActivity).mainViewModel.setCacheBranches()
                    ...
                    if (  (requireActivity() as MainActivity).mainViewModel.cachedBranches.value?.get(0)?.code
                        ==(requireActivity() as MainActivity).mainViewModel.enteredCode.value){
                        (requireActivity() as MainActivity).mainViewModel.getEmployeesNeedsToBeSynced()
    ...
    private fun syncButtonListener()
    {
        binding.sync.setOnClickListener {
           ...
           (requireActivity() as MainActivity).mainViewModel.syncNeedToBeCachedEmployees()
        }
    }
